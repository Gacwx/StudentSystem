...
namespace MyApp.Api.Controllers
{
    ...
    public class StudentsController : ControllerBase
    {
        ...
        public StudentsController(ModelContext db, IMapper mapper) { _db = db; _mapper = mapper; }

        [HttpGet]
        public async Task<ActionResult<IEnumerable<StudentDto>>> GetAll([FromQuery] int? departmentId)
        {
            var q = _db.Students.Include(s => s.Department).AsQueryable();
        ...
        {
            var s = await _db.Students.Include(x => x.Department).FirstOrDefaultAsync(x => x.Id == id);
        ...
        public async Task<ActionResult<StudentDto>> Create([FromBody] StudentCreateDto dto)
        {
            ...
            var created = await _db.Students.Include(d => d.Department).FirstAsync(s => s.Id == entity.Id);
